"use client";
import { Settings, Database, Cpu, Network, LayoutGrid, BookOpen, FlaskConical, Blocks, User, UserCircle, Command } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 border-r border-slate-800/60 bg-[#0F141F] flex flex-col justify-between shadow-2xl z-20">
      <div>
        {/* Logo */}
        <div className="h-16 flex items-center px-6 border-b border-slate-800/60">
          ...
          >
            ...
            <span className="text-white font-bold tracking-wide text-[17px] group-hover:text-white transition-colors duration-300">Turing Hub</span>
          </motion.div>
        </div>
        ...
        <nav className="p-4 flex flex-col gap-1.5 mt-2">
          {navItems.map((item, idx) => {
            ...
            if (isActive) {
              return (
                <Link href={item.href} key={idx} className="flex items-center gap-3 px-4 py-3 bg-cyan-500/10 text-cyan-400 rounded-xl border border-cyan-500/20 cursor-pointer shadow-[0_0_10px_rgba(34,211,238,0.05)] relative overflow-hidden">
                  <motion.div 
                  ...
                  <span className="font-semibold text-sm relative z-10">{t(item.labelKey)}</span>
                </Link>
              );
            }

            return (
              <Link href={item.href} key={idx} className="group flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 rounded-xl cursor-pointer transition-all">
                <item.icon className="w-5 h-5 opacity-70 group-hover:opacity-100 group-hover:scale-110 transition-transform" />
      ...
      <div className="p-4 border-t border-slate-800/60 relative">
          ...
          {isMenuOpen && (
            <>
              ...
              <motion.div 
                ...
                className="absolute bottom-full left-4 right-4 mb-2 py-2 rounded-xl border border-slate-700/80 bg-[#0A0E17]/95 backdrop-blur-xl shadow-[0_0_30px_rgba(0,0,0,0.5)] z-50 flex flex-col min-w-[200px]"
              >
        ...
        <div 
          ...
          className={`flex items-center justify-between px-3 py-2.5 rounded-xl cursor-pointer transition-colors border ${isMenuOpen ? 'bg-slate-800/50 border-slate-700' : 'hover:bg-slate-800/30 border-transparent'}`}
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-cyan-600 to-emerald-600 flex items-center justify-center border border-slate-700 shadow-inner">
              <User className="w-4 h-4 text-white" />
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-slate-200 leading-tight">{t("admin")}</span>
              <span className="text-[10px] text-slate-500 font-medium">Turing Hub</span>
            </div>
          </div>
          <Settings className={`w-4 h-4 text-slate-500 transition-transform duration-300 ${isMenuOpen ? 'rotate-90 text-cyan-400' : ''}`} />
        </div>
      </div>
