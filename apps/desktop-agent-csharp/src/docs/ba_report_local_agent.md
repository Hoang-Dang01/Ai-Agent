...
*   **Các bước thực hiện:**
    ...
    2.  Kịch hoạt Agent thực hiện tác vụ đăng nhập.
*   **Tiêu chí nghiệm thu (Acceptance Criteria):**
...
## 10. PHÂN HỆ CÀO DỮ LIỆU TỰ ĐỘNG CHẠY NGẦM (BACKGROUND SCRAPING & SCHEDULING MODULE)

Để giải quyết bài toán cào dữ liệu (data scraping) định kỳ một cách hoàn toàn tự động, im lặng và không chiếm dụng chuột hay màn hình của người dùng, C# cung cấp giải pháp **Background Service** tối ưu hóa sâu sắc.

### 10.1. Kiến trúc phân hệ cào ngầm (Background Scraping Stack)

*   **Tác vụ tĩnh (HttpClient & HtmlAgilityPack):** Đối với các trang web có cấu trúc tĩnh hoặc API mở, C# sử dụng lớp `HttpClient` gốc để gửi yêu cầu trực tiếp đến máy chủ và phân tích mã HTML bằng `HtmlAgilityPack` chỉ trong vài mili-giây. Phương án này tiêu thụ cực ít tài nguyên (chỉ khoảng 15MB RAM hệ thống, 0% GPU).
*   **Tác vụ động (Playwright .NET Headless):** Đối với các trang web động yêu cầu đăng nhập và render JavaScript, Agent khởi chạy một trình duyệt Chromium ẩn (Headless mode) trong bộ nhớ RAM, tự động tương tác và trích xuất dữ liệu mà không làm dịch chuyển con trỏ chuột vật lý của người dùng.
*   **Cơ chế lập lịch ngầm (PeriodicTimer):** Tận dụng tính năng `BackgroundService` và `PeriodicTimer` của .NET 9.0 để chạy một vòng lặp vô hạn dưới nền hệ thống, tự động kích hoạt tiến trình cào dữ liệu theo chu kỳ định sẵn (ví dụ: mỗi 30 phút hoặc 1 tiếng).
*   **Tự động khởi chạy cùng hệ điều hành (Windows Service):** Ứng dụng C# có thể đăng ký trực tiếp dưới dạng một Windows Service, tự động chạy ngầm ngay khi máy tính khởi động mà không cần người dùng đăng nhập Desktop.

### 10.2. Mã nguồn cấu trúc dịch vụ chạy ngầm (.NET 9.0 BackgroundService Blueprint)
...
namespace OfflineAgent.Core.BackgroundTasks
{
    public class AutoScraperService : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            // Thiết lập chu kỳ tự động chạy: Ví dụ 30 phút/lần
            ...
            while (await timer.WaitForNextTickAsync(stoppingToken))
            {
                try
                {
                    // Thực thi tác vụ cào dữ liệu và ghi vào Database
                    await ExecuteScrapingTaskAsync();
                }
                catch (Exception ex)
                {
                    // Ghi log lỗi ngầm
        ...
        private async Task ExecuteScrapingTaskAsync()
        {
            // Kết nối HttpClient/Playwright để cào dữ liệu và cập nhật DB Medstand
...
## 11. KIẾN TRÚC CỘNG TÁC NHIỀU THỰC THỂ (MULTI-AGENT COLLABORATION ARCHITECTURE)

Để xử lý các quy trình nghiệp vụ phức tạp đòi hỏi sự phối hợp giữa lên kế hoạch, thực thi giao diện hệ thống và đối soát số liệu (ví dụ: cào bảng giá Medstand, đối chiếu công nợ và điền điểm tích lũy), hệ thống cung cấp giải pháp **Multi-Agent Collaboration** dựa trên thư viện **Microsoft Semantic Kernel**.

### 11.1. Sơ đồ các Thực thể Tự hành (Agent Roles & Boundaries)

Hệ thống được vận hành bởi 3 thực thể Agent chuyên biệt chạy ngầm dưới nền thông qua một group chat chung:

*   **Planner Agent (Nhạc trưởng điều phối):**
    *   *System Rules:* Phân tích yêu cầu ngôn ngữ tự nhiên ban đầu của người dùng, phân tách thành danh sách nhiệm vụ con và giao việc cho Executor/Verifier. Tuyệt đối không tự ý chạy các hàm logic. Chỉ tuyên bố hoàn thành khi Verifier xác nhận kết quả là đúng.
*   **OS Executor Agent (Thực thi giao diện):**
    *   *System Rules:* Chỉ thực thi các hành động điều khiển Windows (mở Notepad, nhập dữ liệu WebView2, click nút bấm) khi có yêu cầu cụ thể từ Planner. Bắt buộc sử dụng các công cụ (Plugins) được cung cấp. Báo cáo chi tiết các bước đã thực hiện cho Planner ngay sau khi hoàn thành.
*   **Data Verifier Agent (Kiểm soát viên chất lượng):**
    *   *System Rules:* Độc lập kiểm chứng kết quả thực hiện của Executor bằng cách truy vấn trực tiếp cơ sở dữ liệu SQL nội bộ hoặc quét ảnh chụp màn hình bằng YOLOv8. Tuyệt đối không dựa vào lời nói của Executor. Trả về kết quả ĐÚNG hoặc SAI kèm bằng chứng số liệu cụ thể cho Planner.

### 11.2. Khung vận hành cuộc hội thoại ngầm (Orchestration & Termination Strategy)

Để các Agent tự động trao đổi thông tin, phối hợp logic và tự động kết thúc an toàn, hệ thống cấu hình các chiến lược sau:

*   **Bộ nhớ ngữ cảnh dùng chung (Shared Chat History):** Các Agent cùng đọc và viết vào một đối tượng lịch sử chat chung. Điều này cho phép Verifier tự động bắt đầu đối soát ngay khi đọc được báo cáo hoàn thành của Executor trong lịch sử trò chuyện.
*   **Chiến lược dừng phê duyệt (Approval Termination Strategy):** Để tránh việc các Agent chat ngầm vô hạn (Infinite Loop), hệ thống cấu hình một chiến lược dừng nghiêm ngặt. Cuộc hội thoại sẽ dừng lại ngay lập tức khi Planner đọc được kết quả của Verifier và đưa ra từ khóa xác nhận đặc biệt `[COMPLETED]`, hoặc khi số lượt đối thoại vượt quá hạn mức tối đa (Max Iterations = 10).
*   **Tích hợp Công cụ Tự động (Skills/Plugins Registration):** Các hàm lập trình C# (như ghi Notepad, cào Web, chạy SQL) được đóng gói thành các Plugin bằng cách sử dụng thuộc tính `[KernelFunction]`. Executor Agent sẽ tự động phân tích ngữ cảnh, lựa chọn công cụ tương thích và tự kích hoạt hàm C# (Auto Function Calling) mà không cần lập trình viên viết mã `if/else` thủ công.

### 11.3. Mã nguồn Blueprint Khung Multi-Agent C# (.NET 9.0)
...
namespace OfflineAgent.Core.MultiAgent
{
    public class MultiAgentOrchestrator
    {
        public async Task StartExecutionLoopAsync(string userGoal)
        {
            var kernel = new Kernel();

            // Khởi tạo Planner, Executor, Verifier Agents
            var planner = new ChatCompletionAgent { Name = "Planner", Kernel = kernel };
            var executor = new ChatCompletionAgent { Name = "Executor", Kernel = kernel };
            var verifier = new ChatCompletionAgent { Name = "Verifier", Kernel = kernel };

            // Khởi tạo Chiến lược dừng kiểm tra từ khóa [COMPLETED]
            var stopStrategy = new ApprovalTerminationStrategy
            {
                Agents = [planner],
                MaxIterations = 10 
            };

            // Thiết lập Khung hội thoại chung
            var agentChat = new AgentGroupChat(planner, executor, verifier)
            {
                ExecutionSettings = new() { TerminationStrategy = stopStrategy }
            };

            await agentChat.AddChatMessageAsync(new ChatMessageContent(AuthorRole.User, userGoal));
