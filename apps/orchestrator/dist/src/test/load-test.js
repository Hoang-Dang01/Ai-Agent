...
function startMockPythonBackend(port) {
    ...
    app.post('/api/plan/generate', (req, res) => {
        ...
        const traceId = req.headers['x-trace-id'] || 'mock-trace-id';
        const spanId = req.headers['x-span-id'] || 'mock-span-id';
...
async function runLoadTests() {
    ...
    try {
        ...
        const stressRequests = Array.from({ length: 50 }, (_, i) => {
            return axios_1.default.post('http://localhost:3003/api/goals/plan', { goal: `Concurrent stress test goal number ${i}` }, {
                headers: {
        ...
        const statsRes = await axios_1.default.get('http://localhost:3003/api/telemetry/stats', {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
