...
function simulateSocketHitlResponse(goalId, approved) {
    const active = global.activeProcesses?.get(goalId);
...
async function runHardeningTests() {
    ...
    try {
        ...
        const childShort = (0, child_process_1.spawn)('node', ['-e', 'process.stdin.resume(); setTimeout(() => process.exit(0), 50);']);
        let stdinErrTriggered = false;
        childShort.stdin.on('error', (err) => {
            stdinErrTriggered = true;
        });
        global.activeProcesses.set('goal_test_1', { child: childShort });
        ...
        global.activeProcesses.delete('goal_test_1');
        // ----------------------------------------------------
        ...
        const childLong = (0, child_process_1.spawn)('node', ['-e', 'setInterval(() => {}, 1000);']);
        let isKilled = false;
        ...
        global.activeProcesses.set('goal_test_2', { child: childLong });
        ...
        await delay(100);
        assert(isKilled, 'Spawned C# process was successfully terminated with SIGKILL.');
        // Verify map is cleared
        assert(global.activeProcesses.size === 0, 'activeProcesses global map is cleared successfully.');
    }
    catch (error) {
