...
import { TaskStateMachine } from '../utils/task-state-machine';
...
async function runTests() {
  ...
  try {
    ...
    const client = dbService.client as any;
    ...
    // Verify static rules
    assert(TaskStateMachine.isValidTransition('QUEUED', 'DISPATCHED') === true, 'Static rule allowed: QUEUED -> DISPATCHED');
    assert(TaskStateMachine.isValidTransition('DISPATCHED', 'ACKNOWLEDGED') === true, 'Static rule allowed: DISPATCHED -> ACKNOWLEDGED');
    assert(TaskStateMachine.isValidTransition('FAILED', 'EXECUTING') === false, 'Static rule blocked: FAILED -> EXECUTING');
    assert(TaskStateMachine.isValidTransition('COMPLETED', 'QUEUED') === false, 'Static rule blocked: COMPLETED -> QUEUED');
    ...
    await TaskStateRepository.updateWithFence(stateTask.id, 1, { status: 'DISPATCHED' });
    ...
    try {
      await TaskStateRepository.updateWithFence(stateTask.id, 1, { status: 'QUEUED' }); // DISPATCHED -> QUEUED is invalid
    } catch (err: any) {
    ...
    const node1Success = await TaskStateRepository.updateWithCAS(casTask.id, 1, { status: 'ACKNOWLEDGED' });
    // Node 2 attempts next with outdated epoch (should fail)
    const node2Success = await TaskStateRepository.updateWithCAS(casTask.id, 1, { status: 'ACKNOWLEDGED' });
    ...
    await TaskStateRepository.updateWithFence(dlqTask.id, 1, patchPayload);
