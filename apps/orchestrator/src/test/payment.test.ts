...
const StripeMock = function (key: string, config?: any) {
  const instance = new OriginalStripe(key, config);
  instance.checkout.sessions.create = async function (params?: any) {
    return {
...
import { createCheckoutSession, handleStripeWebhook, getSubscriptionStatus } from '../controllers/payment.controller';
...
async function runTests() {
  ...
  const originalPriceId = env.STRIPE_PRICE_ID;
  const originalWebhookSecret = env.STRIPE_WEBHOOK_SECRET;

  try {
    // Override environment with safe mock credentials
    env.STRIPE_SECRET_KEY = 'sk_test_mock_secret_key_antigravity_123';
    env.STRIPE_PRICE_ID = 'price_mock_premium_pack';
    env.STRIPE_WEBHOOK_SECRET = 'whsec_mock_webhook_signature_secret_456';
    ...
    await handleStripeWebhook(webhookReq, webhookRes);
    ...
    await getSubscriptionStatus(statusReq, statusRes);
    ...
    await dbService.client.userSubscription.update({
      where: { userId: testUserId },
      data: {
    ...
    await getSubscriptionStatus(statusReq, statusExpiredRes);
    ...
    await handleStripeWebhook(cancelWebhookReq, cancelWebhookRes);

    assert(cancelWebhookRes.statusCode === 200, 'Cancel webhook returns 200 OK');

    const updatedSub = await dbService.client.userSubscription.findUnique({
      where: { userId: testUserId },
    });
    ...
    await dbService.client.userSubscription.deleteMany({
      where: { userId: testUserId },
    });
    await dbService.client.user.delete({
      where: { id: testUserId },
    });
    ...
    env.STRIPE_PRICE_ID = originalPriceId;
