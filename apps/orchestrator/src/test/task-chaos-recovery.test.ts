  ...
  try {
    ...
    const results = await Promise.all(
      Array.from({ length: 100 }, () =>
        TaskStateRepository.updateWithCAS(contentionTask.id, 100, { status: 'FAILED' })
      )
    );
    ...
    const metricsA = MetricsCollector.getMetrics();
    ...
    const postRollbackMetrics = MetricsCollector.getMetrics();
    assert(
      postRollbackMetrics.dlq_total === initialMetrics.dlq_total &&
      postRollbackMetrics.fencing_rejection_total === initialMetrics.fencing_rejection_total,
      'Observability metrics were not incremented during aborted transaction.'
    );
    ...
    await TaskStateRepository.updateWithFence(outageTask.id, 200, { status: 'DISPATCHED' });
    ...
    const sweepCountOutage = await OutboxPublisher.publishPendingEvents();
    ...
    const sweepCountRestored = await OutboxPublisher.publishPendingEvents();
    ...
    const reapedCount = await LeaseReaperService.reap();
    ...
    const zombieReapedCount = await LeaseReaperService.reap();
    ...
    try {
      await TaskStateRepository.updateWithFence(
        zombieTask.id,
        BigInt(888), // Old Fencing Token
        { status: 'COMPLETED' },
        undefined,
        'EXECUTING', // Old Expected status
