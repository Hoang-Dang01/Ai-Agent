...
async function runTelemetryTests() {
  ...
  try {
    ...
    const costs1 = calculateTokensCost('gemini-1.5-flash', 1_000_000, 2_000_000);
    ...
    const insertedTrace = await dbService.client.aITelemetryTrace.create({
      data: {
        traceId,
        spanId,
        traceType: 'PLAN',
    ...
    const success95DaysOld = await dbService.client.aITelemetryTrace.create({
      data: {
    ...
    const failed185DaysOld = await dbService.client.aITelemetryTrace.create({
      data: {
    ...
    const failed100DaysOld = await dbService.client.aITelemetryTrace.create({
      data: {
    ...
    const success35DaysOld = await dbService.client.aITelemetryTrace.create({
      data: {
    ...
    const freshTrace = await dbService.client.aITelemetryTrace.create({
      data: {
        traceId: 'fresh-trace',
        spanId: 'fresh-span',
        traceType: 'PLAN',
    ...
    const pruneResults = await runTelemetryRetentionPruning();
    ...
    // Verify survivors
    const sOldCheck = await dbService.client.aITelemetryTrace.findUnique({ where: { id: success95DaysOld.id } });
    const fOldCheck = await dbService.client.aITelemetryTrace.findUnique({ where: { id: failed185DaysOld.id } });
    const f100Check = await dbService.client.aITelemetryTrace.findUnique({ where: { id: failed100DaysOld.id } });
    const s35Check = await dbService.client.aITelemetryTrace.findUnique({ where: { id: success35DaysOld.id } });
    const freshCheck = await dbService.client.aITelemetryTrace.findUnique({ where: { id: freshTrace.id } });
