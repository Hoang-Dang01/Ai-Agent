export interface CostResult {
  calculatedInputCostUsd: number;
  calculatedOutputCostUsd: number;
...
const PRICING_REGISTRY: Record<string, PriceConfig> = {
  "gemini-1.5-flash": {
...
export function calculateTokensCost(
  ...
  const calculatedInputCostUsd = (input / 1_000_000) * config.inputPerMillion;
  const calculatedOutputCostUsd = (output / 1_000_000) * config.outputPerMillion;
