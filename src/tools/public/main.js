        ...
        const modeSelect = `
            <div class="bot-configs" onclick="event.stopPropagation()">
                <div class="config-row">
                    <select class="mode-select" id="mode-${acc.id}" onchange="document.getElementById('pv-config-${acc.id}').style.display = this.value === 'trade' ? 'flex' : 'none';">
                        <option value="afk" ${acc.mode === 'afk' ? 'selected' : ''}>🛡️ Smart AFK</option>
                        <option value="trade" ${acc.mode === 'trade' ? 'selected' : ''}>💰 Auto Trade</option>
                        <option value="random" ${acc.mode === 'random' ? 'selected' : ''}>🚶 Tản ra</option>
                    </select>
                    <label class="pvp-toggle">
                        <input type="checkbox" id="pvp-${acc.id}" ${acc.pvpEnabled !== false ? 'checked' : ''}> 
                        <span>PvP</span>
                    </label>
                </div>
                <div class="config-row">
                    <select class="mode-select" id="afkspot-${acc.id}" title="Chọn góc AFK">
                        <option value="">-- Chọn Góc AFK --</option>
                        ${afkSpotOptions}
                    </select>
                    <button class="btn-save-config" onclick="saveBotConfig('${acc.id}', this)" title="Lưu cấu hình">💾</button>
                </div>
                <div class="config-row" id="pv-config-${acc.id}" style="display: ${acc.mode === 'trade' ? 'flex' : 'none'}; gap: 5px;">
                    <span style="font-size: 11px; color: #ccc;">Kho PV:</span>
                    <input type="number" id="pvstart-${acc.id}" value="${acc.pvStart || 2}" style="width: 40px; background: rgba(0,0,0,0.3); color: #fff; border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 2px 4px; font-size: 11px;"> 
                    <span style="color: #ccc;">-</span> 
                    <input type="number" id="pvend-${acc.id}" value="${acc.pvEnd || 5}" style="width: 40px; background: rgba(0,0,0,0.3); color: #fff; border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 2px 4px; font-size: 11px;">
                </div>
            </div>
        `;
            ...
            <div class="bot-card ${isSelected}" id="card-${acc.id}" onclick="toggleSelectBot('${acc.id}', '${acc.username}')">
                <div class="bot-header">
                    <div>
                        ...
                        <p>Pass: ${'*'.repeat(acc.pass.length)} | Server: Spring</p>
                    </div>
                </div>
                ${modeSelect}
                <div class="bot-actions" id="actions-${acc.id}">
                    ${actions}
                    <button class="btn btn-del" onclick="event.stopPropagation(); deleteBot('${acc.id}')" title="Xoá Account"><i class="fa-solid fa-trash"></i></button>
                </div>
            </div>
        `;
...
function renderLocations() {
    ...
    if (globalLocations.npc_trade) {
        html += `<span class="loc-tag" style="background: rgba(236, 72, 153, 0.15); color: #f472b6; border-color: rgba(236, 72, 153, 0.3);">
    ...
    if (globalLocations.afk_spots && globalLocations.afk_spots.length > 0) {
        globalLocations.afk_spots.forEach(spot => {
            ...
            <span class="loc-tag">
                ...
                <i class="fa-solid fa-xmark hover-red" onclick="deleteLocation('${spot.id}')" title="Xoá"></i>
            </span>`;
        });
    }
